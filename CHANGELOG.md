...
### Bug Fixes
- **Multi Control Init script** — now explicitly stops `SamsungSystemSupportEngine` and restarts `GBeSupportService` before the Samsung wildcard service loop. Previously, the `*Samsung*` wildcard missed `GBeSupportService` (GBE's patched service), leaving Samsung apps showing stale model data (e.g. `960QHA`) after a reboot even though the registry spoof was correct. Service restart order: stop engine → restart GBeSupportService → restart Samsung services.
